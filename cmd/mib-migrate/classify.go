...
// Confidence labels each plan entry; low-confidence rows are routed
// to mibs/unsorted/ for the maintainer to re-classify by hand.
...
// destination directory per design.md Decision 9.
type Classification struct {
	...
	Vendor string
	// Confidence is the migration tool's self-rating for this entry.
	Confidence Confidence
}

// Classify maps a MIB's MODULE-IDENTITY OID + module name to a corpus
// destination. The rules (per design.md Decision 9):
//
...
// migration-time slug overrides, then iana.Slug applied to the
// upstream registry name. Returns ("unknown", medium) if the PEN
// isn't in the curated registry — the maintainer can hand-edit the
// plan TSV before apply.
